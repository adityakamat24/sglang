...
class ServerArgs:
    ...
    def _handle_speculative_decoding(self):
        ...
        if self.speculative_algorithm == "SUFFIX":
            ...
            from sglang.srt.utils.import_utils import has_arctic_inference
            ...
            if not self.device.startswith("cuda"):
                raise ValueError(
                    "Suffix decoding only supports CUDA device."
                )
            ...
            self.enable_mixed_chunk = False
            if self.speculative_num_draft_tokens is None:
                self.speculative_num_draft_tokens = self.speculative_suffix_max_tree_depth
                logger.warning(
